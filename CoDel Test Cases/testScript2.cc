...
CoDelPeekTest::DoRun(){
  ...
  Enqueue(queue, pktSize, 10);
  NS_LOG_UNCOND("Enqueue Done");
  Time firstDequeueTime = 0.5 * queue->GetTarget();
  ...
  Time nextDequeueTime = firstDequeueTime + 2 * queue->GetTarget();
  Simulator::Schedule(nextDequeueTime, &CoDelPeekTest::Dequeue, this, queue);
  Simulator::Schedule(nextDequeueTime*1.5, &CoDelPeekTest::Dequeue, this, queue);
  nextDequeueTime = firstDequeueTime + 2 * queue->GetInterval();
  Simulator::Schedule(nextDequeueTime, &CoDelPeekTest::Dequeue, this, queue);
  Simulator::Schedule(nextDequeueTime *1.5, &CoDelPeekTest::Dequeue, this, queue);
  Simulator::Schedule(nextDequeueTime *1.5, &CoDelPeekTest::Peek, this, queue);
  Simulator::Run();
  Simulator::Destroy();
...
CoDelPeekTestMark::DoRun(){
  ...
  Simulator::Schedule(nextDequeueTime*1.5, &CoDelPeekTestMark::Dequeue, this, queue);
  nextDequeueTime = firstDequeueTime + 2 * queue->GetInterval();
  Simulator::Schedule(nextDequeueTime, &CoDelPeekTestMark::Dequeue, this, queue);
  Simulator::Schedule(nextDequeueTime *1.5, &CoDelPeekTestMark::Dequeue, this, queue);
  Simulator::Schedule(nextDequeueTime *1.5, &CoDelPeekTestMark::Peek, this, queue);
  Simulator::Run();
  Simulator::Destroy();
}
...
int main(){
    NS_LOG_UNCOND("------------------------------");
    NS_LOG_UNCOND("Test 2 for CoDel without Marking");
    NS_LOG_UNCOND("------------------------------");
    ...
    NS_LOG_UNCOND("-------------------------------------");
    NS_LOG_UNCOND("Test 2 for CoDel with Marking(only ECN)");
    NS_LOG_UNCOND("-------------------------------------");
