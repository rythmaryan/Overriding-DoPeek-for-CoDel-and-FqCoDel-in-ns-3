...
CoDelPeekTest::DoRun(){
  ...
  Time nextDequeueTime =  2 * queue->GetTarget();
  Simulator::Schedule(nextDequeueTime, &CoDelPeekTest::Dequeue, this, queue);
  Simulator::Schedule(queue->GetTarget()*5, &CoDelPeekTest::Dequeue, this, queue);
...
CoDelPeekTest::Enqueue(Ptr<CoDelQueueDisc> queue, uint32_t pktSize, uint32_t nPkt){
  Address addr;
   NS_LOG_UNCOND("Enqueue Done -"<<nPkt<<" Packets");
...
CoDelPeekTestMark::DoRun(){
  ...
  Simulator::Schedule(queue->GetTarget()*5, &CoDelPeekTestMark::Dequeue, this, queue);
  nextDequeueTime =  nextDequeueTime + queue->GetInterval();
...
int main(){
    NS_LOG_UNCOND("------------------------------");
    NS_LOG_UNCOND("Test 4 for CoDel without Marking");
    NS_LOG_UNCOND("------------------------------");
    ...
    NS_LOG_UNCOND("-------------------------------------");
    NS_LOG_UNCOND("Test 4 for CoDel with Marking(only ECN)");
    NS_LOG_UNCOND("-------------------------------------");
